...
import com.progressoft.jfw.annotations.processor.copier.AnnotatedClassWithPublicCollections.AnotherObject;
...
class AnnotatedClassWithPublicCollectionsCopier {
    ...
    AnnotatedClassWithPublicCollections copy(AnnotatedClassWithPublicCollections original) throws CloneNotSupportedException {
        ...
        if(java.util.Objects.nonNull(original.anotherObjectArray)){
            result.anotherObjectArray=new AnotherObject[original.anotherObjectArray.length]);
            for(int i=0;i<original.anotherObjectArray.length;i++){
                result.anotherObjectArray[i]=original.anotherObjectArray[i];
            }
        }
